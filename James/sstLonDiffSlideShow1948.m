function [F] = sstLonDiffSlideShow1948(start_month, end_month, start_year,end_year)
...
[annualSST,lat, lon] = getAnnualSSTAnomalies(1, 1, start_year, end_year);

annualSSTMat = zeros(size(annualSST, 1), size(annualSST, 2), size(annualSST, 3), 12);

parfor i = 1:12
    annualSSTMat(:, :, :, i) = getAnnualSSTAnomalies(i, i, start_year, end_year);
...
F = makeSlideShow(maxI, maxJ, minI, minJ, annualSSTMat, lat, lon,start_year,end_year);
    ...
    for month = 1:size(maxI, 2)
        for year = 1:size(maxI, 1)
            plotCurrentBox(lat, lon, maxI(year, month), maxJ(year, month), minI(year, month), ...
                minJ(year, month), annualSSTMat(:, :, year, month), month, year, start_year, end_year);
...
saveDir = ['/project/expeditions/jfagh/code/matlab/ClimateCode/james/results/sst_dist_plots/slide' num2str(years(year)) monthStr '.pdf'];
