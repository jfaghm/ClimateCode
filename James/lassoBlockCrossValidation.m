...
function [B, F, y_pred fold_mse, result] =  lassoBlockCrossValidation(x,y,k, lambda)
...
for i=1:size(x,1)-k+1
    ...
    result(count,i:i+k-1) = y_pred{count};
    count = count+1;
    
end
y_pred = cell2mat(y_pred'); %this is meaningless for block
end