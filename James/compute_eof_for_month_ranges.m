...
for k =1:12:size(pacificBasin,3)
    count = 1;
for i =1:12
    for j=i:12
        data{year, count} =  nanmean(pacificBasin(:, :, k+(i-1):k+(j-1)), 3);
        count = count+1;
...
for i=1:size(new_data,4)
    input = squeeze(new_data(:,:,:,i));
    
    [maps{i}, PCs{i}] = eof(input, 1);
end
