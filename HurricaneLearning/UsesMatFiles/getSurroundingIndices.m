function neighbors = getSurroundingIndices( numRows, numCols, index, neighborWidth )
%GETSURROUNDINGINDICES Gets the indices of the entries surrounding index entry.
%   getSurroundingIndices gets the indices of A which surround the element at the
%   position given by index.  The index given is assumed to be a column
%   major index into a 2D matrix.  The matrix A is treated as if it is a
