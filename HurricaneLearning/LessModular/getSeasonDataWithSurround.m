function [ out ] = getSeasonDataWithSurround(indices, season)
...
numLocations = (surroundWidth*2 + 1)^2;
...
for i = 1:season.numStorms
    ...
    % set up the contents of the first four columns of each row
    posHeader = zeros(1, 4, numLocations);
    negHeader = zeros(1, 4, numLocations);
    for loc = 1:numLocations
        posHeader(:, :, loc) = [1 dayNumber season.lat(i) season.lon(i)];        
    end
    
  
    out{4*(i-1)+1, 1} = posHeader;
 
    
    % get the date strings for the storm
    dateStrings = generateDateStrings(yearString, season.months(i), season.days(i));
    
    % store the index into the grid
    index = indices(i);
    % get the surrounding indices in a vector
    surrounding = getSurroundingIndices(256, 512, indices(i), surroundWidth);
    ...
    for dateNum = 1:(numDates-1)
        ...
        for loc = 1:numLocations
            negHeader(:, :, loc) = [0 dayNumber season.lat(i) season.lon(i)];
        end
        % set info for that date for each surrounding location
        out{4*(i-1) + dateNum + 1, 1} = negHeader;
    ...
    for dateNum = 1:numDates
        ...
        sstArray = zeros(1, 10, numLocations);
        for j = 1:10
            if strcmp(dateStrings{dateNum, j}, 'INVALID')
                sst = ones(1, numLocations)*-1;
            else
        ...
        for j = 1:10
            ...
            if strcmp(dateStrings{dateNum, j}, 'INVALID')
                ...
                try
                    ...
                    fprintf('%s\n'negHeader(:, :, loc) = [0 dayNumber season.lat(i) season.lon(i)];, filename)
                    fprintf('%s\n', ME.message)
                    uWind = ones(1, numLevels) * -1;
                    vWind = ones(1, numLevels) * -1;
                end
        ...
        for j = 1:10
            ...
            if strcmp(dateStrings{dateNum, j}, 'INVALID')
                ...
                try
                    ...
                    mslpArraCHANTALy = netcdf.getVar(ncid, varID)';
